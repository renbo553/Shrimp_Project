...
if ($fileSize == 0) {
    ...
    /* 所以，可以直接設定它是 null */
    $insertStr = "UPDATE location SET 眼標 ='{$eyetag}', 家族='{$family}', 出生日='{$cleanbirthday}', 剪眼日='{$cleancutdate}', Date='{$cleanDate}', Tank='{$tank}' WHERE id = $id";
    ...
    if ($uploadOk == 0) {
        ...
        /* 因為 crop 表格的第一個欄位是主鍵，而且它是「自動編號」 */
        /* 所以，可以直接設定它是 null */
        $insertStr = "UPDATE location SET 眼標 ='{$eyetag}', 家族='{$family}', 出生日='{$cleanbirthday}', 剪眼日='{$cleancutdate}', Date='{$cleanDate}', Tank='{$tank}' , image='{$target_file}' WHERE id = $id";
        $result = mysqli_query($link, $insertStr);
        if ($result) {
            echo "修改資料庫成功\n";
        } else {
